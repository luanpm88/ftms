    ...
    list.each do |row|
      if !row[:contacts].empty?
        ...
        row[:contacts].each do |contact|
          if row[:name].mb_chars.strip.downcase == contact.name.mb_chars.strip.downcase && row[:email].strip.downcase == contact.email.strip.downcase && Contact.format_mobile(row[:mobile]) == contact.mobile
