class BooksContact < ActiveRecord::Base
  ...
  def paid_amount(from_date=nil, to_date=nil)
    ...
    records.each do |p|
      ...
      prds = prds.joins(:payment_record) if from_date.present? || to_date.present?
  ...
  def self.datatable(params, user)
    ...
    @records = @records.joins(:contact, :course_register, :book => [:course_type, :subject, :stock_type])
    ...
    if !params["search"]["value"].empty?
      @records = @records.includes(:book => [:course_type, :subject, :stock_type])
  ...
  def display_cancel_books_contact
    if !canceled
      '<div class="cancel-books-contact"><a class="badge badge-success cancel-books-contact-button" href="#cancel">Cancel</a><div class="form"><input type="hidden" value="' + self.id.to_s + '" name="books_contact_id" /><label>Reason:</label><textarea name="reason"></textarea><button>Save</button></div></div>'
    else
