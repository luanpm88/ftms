class CourseTypesSubject < ActiveRecord::Base
  ...
  def self.full_text_search(q)
    result = self.joins("LEFT JOIN course_types as course_types_2 ON course_types_2.id=course_types_subjects.course_type_id")
    ...
    result = result.limit(50).map {|model| {:id => model.display_id, :text => model.display_name} }
  end
