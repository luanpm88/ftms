module CoursesHelper
  def render_course_actions(item)
    actions = '<div class="text-right"><div class="btn-group actions">
      ...
      if can? :update, item
        actions += '<li>'+ActionController::Base.helpers.link_to('Edit', {controller: "courses", action: "edit", id: item.id, tab_page: 1}, psrc: courses_path(tab_page: 1), title: "Edit: #{item.name}", class: "tab_page")+'</li>'        
      end
