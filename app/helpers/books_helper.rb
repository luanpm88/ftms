module BooksHelper
  def render_book_actions(item)
    actions = '<div class="text-right"><div class="btn-group actions">
      ...
      if can? :update, item
        actions += '<li>'+ActionController::Base.helpers.link_to('Edit', {controller: "books", action: "edit", id: item.id, tab_page: 1}, psrc: books_url(tab_page: 1), title: item.name, class: "tab_page")+'</li>'        
      end
